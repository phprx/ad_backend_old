...
import pandas as pd
import core.ad_model.index2.prepare_outputs as po
# import core.ad_model.index2.visualize as vis
import core.ad_model.index2.prepare_inputs as pi
import core.ad_model.index2.generalized_mallows as gm
import core.ad_model.index2.data_likelihood as dl
import core.ad_model.index2.do_classification as dc
import core.ad_model.index2.gaussian_mixture_model as gmm
...
def parse_inputs(DataIn, MethodOptions, VerboseOptions, Factors, Labels, DataTest, Groups, algo_type):
    if algo_type == 'debm':
        ...
        DMO.JointFit = 0;
        DMO.MixtureModel = 'GMMvv2';
        DMO.Bootstrap = 0;
        DMO.PatientStaging = ['exp', 'p'];
    elif algo_type == 'ebm':
        DMO = namedtuple('DefaultMethodOptions',
    ...
    pdData_all, pdDataTest_all, BiomarkersList, GroupValues = pi.CorrectConfounders(pdData_all, pdDataTest_all,
...
def bootstrap_data_prep(data_CN_raw, data_MCI_raw, data_AD_raw, ptid_CN_raw, ptid_MCI_raw, ptid_AD_raw, Data_all,
                        pdData_all, DMO, DVO, BiomarkersList, GroupValues_cn, GroupValues_ad, GroupValues_mci):
    data_AD_raw_list = []
    data_CN_raw_list = [];
    data_all_list = [];
    ptid_all_list = []
    ...
    if DMO.MixtureModel[:3] == 'GMM':
        ...
        if DMO.MixtureModel == 'GMMvv2':
            ...
            while flag_stop == 0:  # 先训练高斯分布参数，再训练混合模型参数
                ...
                params_opt, bnds_all = gmm.GMM_Control(Data_all, Ncni, Nadi, params_optmix, type_opt=3,
...
def do_patient_staging(pi0, event_centers, DMO, p_yes, p_no, likeli_post, likeli_pre, Groups, GroupValues):
    ...
    if len(Groups) == 0:
        ...
        idx_valid = ~np.isnan(gval)
        gval = gval[idx_valid]
        subj_stages = np.zeros(Y.shape[0]);
        count_gr = -1;
        for g in gval:
            count_gr = count_gr + 1;
            idx = GroupValues[0] == g
...
def show_outputs(Data_all, Data_test_all, pdData_all, Labels, pdDataTest_all, subj_stages, subj_stages_test,
                 BiomarkerParams, evn_full, evn, BiomarkersList, pi0_all, pi0_mean, DMO, DVO, Groups, GroupValues,
                 algo_type):
    ## Visualize Results
    # if DVO.Ordering == 1:
    #     if len(Groups) == 0:
    #         vis.Ordering(BiomarkersList, pi0_all, pi0_mean, DVO.PlotOrder);
    #         if algo_type == 'debm':
    #             vis.EventCenters(BiomarkersList, pi0_mean, evn_full, evn);
    #     else:
    #         gval = np.unique(GroupValues[0])
    #         idx_valid = ~np.isnan(gval)
    #         gval = gval[idx_valid]
    ...
    #             vis.Ordering(BiomarkersList, pi0_all_g, pi0_mean[g], DVO.PlotOrder);
    #             if algo_type == 'debm':
    #                 vis.EventCenters(BiomarkersList, pi0_mean[g], evn_full[g, :], evn[:, g, :]);
    #
    # if DVO.Distributions == 1:
    #     if len(Groups) == 0:
    #         params_all = [BiomarkerParams];
    #         vis.BiomarkerDistribution(Data_all, params_all, BiomarkersList);
    #     else:
    ...
    #             vis.BiomarkerDistribution(Data_all[idxgr, :, :], params_all, BiomarkersList);
    #
    # if DVO.PatientStaging == 1:
    #     print('Estimated patient stages of subjects in training set')
    #     vis.Staging(subj_stages, pdData_all['Diagnosis'], Labels)
    #     if len(Data_test_all) > 0:
    #         print('Estimated patient stages of subjects in test set')
    #         vis.Staging(subj_stages_test, pdDataTest_all['Diagnosis'], Labels)
    return
...
def patient_staging(pi0, event_centers, likeli_post, likeli_pre, type_staging):
    ...
    for j in range(m[1] + 1):  # np.nanprod当axis=1时横向相乘,空值设为1，p_yes_perm[:,:j]列数逐渐增多，p_no_perm[:,j:]列数逐渐减小
        prob_stage[:, j] = pk_s[j] * np.multiply(np.nanprod(p_yes_perm[:, :j], axis=1),
    ...
    if type_staging[0] == 'exp':
        ...
        for i in range(prob_stage.shape[0]):
            ...
            subj_stages[i] = np.mean(np.multiply(np.append(prob_stage[i, 0], pr[~idx_nan]),
