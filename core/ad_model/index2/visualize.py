...
def BiomarkerDistribution(Data_all, params_all, BiomarkersList):
    ...
    for i in range(m[1]):
        ...
        Dallis = Dalli[valid_data].reshape(-1, 1);
        Dallis = Dallis[:, 0];
...
def EventCenters(labels, pi0_mean, evn_full, evn):
    fig, ax = plt.subplots(1, 1, figsize=(7, 7))
    for i in range(len(labels)):
        (_, caps, _) = ax.errorbar(evn_full[i], len(labels) - 1 - pi0_mean.index(i), xerr=np.std(evn[:, i], axis=0),
                                   fmt='--o', color='b', alpha=0.7, barsabove=True, capsize=1)
        for cap in caps:
...
def Staging(subj_stages, Diagnosis, Labels):
    ...
    idx_cn = idx_cn[0]
    idx_ad = np.where(Diagnosis == np.max(Diagnosis));
    idx_ad = idx_ad[0]
    idx_mci = np.where(np.logical_and(Diagnosis > 1, Diagnosis < np.max(Diagnosis)));
    idx_mci = idx_mci[0]
